...
int main(int argc, char** argv)
{
	...
	if (atoi(argv[1]) == 1) {
		...
		if (me == 0) {
			vector<int> a, b, c, intermediary(2 * COEF - 1);
			...
			for (int i = 1; i < nrProcs; i++) {
				...
				begin = (COEF * i) / nrProcs;
				end = (COEF * (i + 1)) / nrProcs;
				//send
				MPI_Ssend(&begin, 1, MPI_INT, i, 4, MPI_COMM_WORLD);
				MPI_Ssend(&end, 1, MPI_INT, i, 5, MPI_COMM_WORLD);
			...
			//int begin = (COEF * me) / nrProcs;
			//int end = (COEF * (me + 1)) / nrProcs;
			int begin, end;
			MPI_Status status;
